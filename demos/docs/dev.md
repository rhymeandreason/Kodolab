...
**Two checkers are slow and almost never need running**, so a bare run skips them: `diffusion/check-diffusion.js` (~65 s) and `folding/tools/check-folding.js` (~45 s). `kit/ribbon.js` does not run the folding one, which reads only its `parseBackbone` and `dssp`. After changing either, run `folding/tools/check-folding.js` by name: it is the only check of DSSP against an experiment (1VII's HELIX records). `tools/check.js`'s `IGNORE` holds exclusions like this one, and `ALSO` the files a checker judges without naming them (a protein's data, the SDF inputs, the DNA pages). A new baked input or a page a checker asserts against gets a line there.
