...
`m.at(name)` is an anchor's world point, or null when that part is off stage, for pinning a page's own chrome (a `kit/badge.js` badge). Anchors for `note()`: `complex.I`, `complex.II`, `complex.III`, `complex.IV`, `quinone`, `cytc`, `synthase`, `leak`, `translocase`, `porin`, `cytosol` (each only when on stage; `complex` is complex I), `pump`, `pump.atp`, `cell.outside` (span `'cell'`), `oxygen` (while one is docked), `H`, `water`, `heads`, `tails`, `outside`, `inside`.
