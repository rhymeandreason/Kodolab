...
| `sides.inside / .outside` | what to call the two compartments; name them from these rather than typing your own |
...
const m = ElectronTransport.mount(el, {
  ...
  proteins: { complex:{ x:-60 }, synthase:{ x:80 }, leak:null },   // omitted: a layout that fits the span. `complex` is the chain's centre, spread into I–IV
  contents: { inside:{ water:30, H:30 }, outside:{ water:30, H:30 } },   // 'H' is a proton. Omitted, the chain gets these 30 a side; H:0 means none
...
**The whole chain is always on stage**: complexes I to IV, ubiquinone and cytochrome c, because "something burns fuel and pumps protons" is a claim the four machines make better than one blob does, and a step that only wants the gradient can point at `synthase` and leave the chain unnamed. The one dial is `span`. Turn it outward when the question is where the ATP goes: `'mitochondrion'` to show it leaving, `'cell'` to show it spent. `span` is `set()` on the same box; to or from `'cell'` rebuilds the box and restarts the counts, keeping every `on()` listener.
...
| `complexLabel`, `complexCaption`, `complexT` | the beat of the lead complex's six-phase cycle, and the words for it |
| `complexStarved` | fuelled, but no protons in the matrix, so it cannot turn |
| `span`, `outerMembrane`, `sides.beyond`, `sides.pumpedInto` | how far out the stage reaches, whether the lid is on, what to call the space above it, and where the protons collect |
| `complexes.I … .IV` `{ turns, pumpsPerTurn, pumped, label }` | each complex's ledger and the beat it is on |
| `protonsPerFuel.NADH / .FADH2` | protons into the intermembrane space per fuel, from the chain's table |
| `shuttles.ubiquinol / .cytcLoaded` | electrons in transit; both stuck full is a chain backed up |
...
`m.at(name)` is an anchor's world point, or null when that part is off stage, for pinning a page's own chrome (a `kit/badge.js` badge). Anchors for `note()`: `complex.I`, `complex.II`, `complex.III`, `complex.IV`, `quinone`, `cytc`, `synthase`, `leak`, `translocase`, `phosphate` (the phosphate carrier beside it), `porin`, `cytosol` (each only when on stage; `complex` is complex I), `pump`, `pump.atp`, `cell.outside` (span `'cell'`), `oxygen` (while one is docked), `H`, `water`, `heads`, `tails`, `outside`, `inside`.
...
const m = LightReactions.mount(el, {
  ...
  cyclic: 0,                  // the share of PSI's turns that send their electrons back round to plastoquinone instead of on to NADP⁺. LightReactions.cyclicToBalance() is the share that lands ATP per NADPH on the Calvin cycle's
  proteins: { complex:{ x:-60 }, synthase:{ x:80 }, leak:null },   // omitted: a layout that fits. `complex` is the chain's centre, spread into PSII, b6f and PSI
  contents: { inside:{ water:30, H:30 }, outside:{ water:30, H:30 } },   // 'H' is a proton. Omitted, the chain gets these 30 a side; H:0 means none
  sideLabels: true,           // both halves named on the stage; false only if you have your own
...
**Light is the fuel and there is no token for it**: nothing arrives at the photosystem and nothing leaves it, which is correct rather than missing. `fuelRate` is the dimmer and `fuel:null` is the dark, where everything stops. `m.feed('light')` is one flash: PSII turns once and PSI owes one turn when its plastocyanins arrive, so switch the supply off, flash once, and watch one pair of electrons go the whole way. `m.feed('light', { cyclic: true })` is a flash whose pair goes round PSI once before it reaches NADP⁺: b6f pumps for it twice, and it costs two more photons. That pair of buttons is the tally a step about the Calvin cycle's bill wants: `LightReactions.CALVIN` is what one CO₂ costs, and a dark thylakoid flashed by hand shows that linear flashes alone leave the ATP short.

**THE WHOLE CHAIN IS ALWAYS ON STAGE**, PSII, b6f and PSI with their shuttles, because light pumping protons is a claim the three machines make and one blob cannot: the pump is not the thing the light hits. A step that only wants the gradient points at `synthase` and leaves the rest unnamed. A photon lands on the green antenna belt behind a photosystem, the excitation hops into the reaction centre, and an electron is thrown out; something then refills the hole, and that is the whole idea. At PSII the refill comes from water on its lumen face: the oxygen-evolving complex stores one oxidizing equivalent per photon (its S state is on a pill) and at four takes all four electrons from two bound waters at once, so **one O₂ and four H⁺ appear every two turns, the protons as real protons in the gradient**, and the O₂ slips across the bilayer and off through the stroma. Plastoquinone carries the pair in the membrane to cytochrome b6f, **the only pump**; plastocyanin carries them one at a time through the lumen to cytochrome f's knob and on to PSI, which a second photon drives, throwing each electron out to its stroma ridge where ferredoxin takes it to FNR; NADP⁺ docks at FNR and leaves as NADPH once two ferredoxins have arrived, taking a stroma proton. So the gradient is built two ways, which is the claim a lumped chain cannot make. b6f slows as the lumen acidifies; the photosystems do not, and stall when every plastoquinone is waiting full. Per pair and per O₂ is `state().protonsPerFuel.light`, `state().light.protonsPerO2` and `.photonsPerO2`; print those, never a typed 6, 8 or 12.

**The ATP is spent where it is made, and it costs more than a mitochondrion's.** The F1 head stands in the stroma, and so does the Calvin cycle, so there is no translocase, no export cost and no `span`: every third of a turn a labelled ATP leaves the head into the stroma and drifts off. This synthase turns on a c14 ring, drawn with fourteen rods, so `state().stoichiometry.protonsPerATP` is about 4.7 where a mitochondrion's is 3; print `stoichiometry.protonsPerATP` and `.ringSubunits` rather than a number remembered from respiration.

**Cyclic electron flow is why the ATP balances the NADPH.** Linear flow makes one NADPH per pair and, on this ring, about 1.3 ATP with it; the Calvin cycle spends 3 ATP per 2 NADPH. Set `cyclic` above 0 and that share of PSI's turns send their electrons back to plastoquinone at PSI's stroma face instead of to NADP⁺, so b6f pumps for them a second time and no NADPH is made: more ATP, no NADPH, and two more photons spent. `LightReactions.cyclicToBalance()` is the share that lands the ratio on the Calvin cycle's, and `state().light.atpPerNADPH` carries `.linear`, `.expected` at the current share, `.calvin`, and `.measured` for this run. A step that asks why there are two ways for the electrons to go, or where the extra ATP comes from, wants a `cyclic` toggle and those numbers, never a typed 1.5.

b6f carries protons **stroma → lumen only**, on a six-phase cycle it visibly turns through. `synthase` is a turbine, not a pump: protons come back up through it and the rotor turns, and it cannot run uphill. `leak` is an uncoupler's hole: protons home without making ATP.
...
| `sides.pumpedInto` | the lumen by name |
| `complexes.PSII / .b6f / .PSI` `{ turns, pumpsPerTurn, pumped, label }` | each machine's ledger and the beat it is on |
| `protonsPerFuel.light` | protons into the lumen per pair, water's included, from the chain's table |
| `shuttles.plastoquinol / .plastocyaninLoaded / .ferredoxinLoaded` | electrons in transit; the first two both stuck full is a chain backed up |
| `light.photons / .waterSplit / .o2Released / .nadphMade / .protonsFromWater / .protonsToNADPH / .cyclicTurns`, `light.photonsPerPair / .protonsPerO2 / .photonsPerO2` | the light reactions' ledger, counted, and their ratios |
| `light.oec.stored / .of / .watersBound` | oxidizing equivalents the oxygen-evolving complex holds toward the next O₂, and the waters bound waiting for it |
| `light.cyclic`, `light.cyclicToBalance`, `light.atpPerNADPH.linear / .expected / .calvin / .measured` | the cyclic share now, the share the Calvin cycle needs, and ATP per NADPH four ways: linear flow alone, at the current share, what the Calvin cycle spends, and this run's count |

Events: `frame` · `pumped` (n) protons into the lumen so far · `atp` (n) · `atpOut` (n) · `oxygen` (n O₂ released) · `nadph` (n made) · `cyclic` (n turns gone round again) · `conduct` (traveller, dir).

`m.at(name)` is an anchor's world point, or null when that part is off stage. Anchors for `note()`: `psii`, `antenna`, `water.split`, `b6f`, `psi`, `plastoquinone`, `plastocyanin`, `ferredoxin`, `fnr`, `nadph`, `oxygen`, `synthase`, `leak` (when on stage; `complex` is b6f), `H`, `water`, `heads`, `tails`, `outside`, `inside`.
