...
const m = LightReactions.mount(el, {
  ...
  cyclic: 0,                  // split: the share of PSI's turns that send their electrons back round to plastoquinone instead of on to NADP⁺. LightReactions.cyclicToBalance() is the share that lands ATP per NADPH on the Calvin cycle's
...
| `light.photons / .waterSplit / .o2Released / .nadphMade / .protonsFromWater / .protonsToNADPH / .cyclicTurns`, `light.photonsPerPair / .protonsPerO2 / .photonsPerO2` | split only: the light reactions' ledger, counted, and their ratios |
| `light.oec.stored / .of / .watersBound` | split only: oxidizing equivalents the oxygen-evolving complex holds toward the next O₂, and the waters bound waiting for it |
| `shuttles.ferredoxinLoaded` | split only: ferredoxins carrying an electron away from PSI |
| `light.cyclic`, `light.cyclicToBalance`, `light.atpPerNADPH.linear / .expected / .calvin / .measured` | split only: the cyclic share now, the share the Calvin cycle needs, and ATP per NADPH four ways: linear flow alone, at the current share, what the Calvin cycle spends, and this run's count |
...
`m.at(name)` is an anchor's world point, or null when that part is off stage. Anchors for `note()`: `complex`, `synthase`, `leak` (when on stage), `psii`, `antenna`, `water.split`, `b6f`, `psi`, `plastoquinone`, `plastocyanin`, `ferredoxin`, `fnr`, `nadph`, `oxygen` (split; `complex` is b6f), `H`, `water`, `heads`, `tails`, `outside`, `inside`.
