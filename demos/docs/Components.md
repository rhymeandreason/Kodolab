...
Parts, each with a card: `outer`, `inner`, `ims`, `stroma`, `granum`, `thylakoid`, `lamella`, `lumen`, `psii`, `b6f`, `psi`, `synthase`, `starch`, `dna`, `ribosome`, `proton`, `oxygen`, `photon`. **Four can be pointed at but not hidden** — `ims`, `stroma` and `lumen` are spaces and `thylakoid` is one disc of a granum — so they take a `notes` chip and never appear under `layers`. **`granum`, `thylakoid`, `lamella`, `psii`, `b6f`, `psi`, `synthase`, `proton`, `oxygen` and `photon` declare a view**, so a `zoom` chip for one of those travels; a `notes` chip only labels.
