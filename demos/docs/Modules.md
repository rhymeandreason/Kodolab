...
| `mol-small.js` · `mol-aminoacids.js` · `mol-pathways.js` · `mol-krebs.js` · `mol-carriers.js` · `mol-heme.js` · `mol-sugars.js` · `mol-glycans.js` · `mol-lipids.js` · `mol-nucleic.js` | nothing — each `register()`s its specs into `MolLib.MOLECULES`. `mol-heme.js` is WRITTEN by `tools/bake-heme.js` from the myoglobin pocket bake, hydrogens grown by atom name; rerun the baker, never edit it | MolecularGeometry.md §1.2, §1.5 |
