...
| `chemiosmosis/circuit.js` | `Chemiosmosis.create` / `Chemiosmosis.mount`, added to the arithmetic's own global: sheet.js plus complex, synthase, uncoupler, translocase, outer membrane, and the ATP, fuel and O₂ tokens. **Load after `membrane/sheet.js` and before `membrane/membrane.js`** | own header |
