...
| `proteins/atp-synthase/atp-synthase-scene.js` | `ATPSynthase.mount(el, {spin})` → `{box, ready, spin, advance, destroy}` · `attach(box)` → `enter(t)` · `exit` · `spin`. The human enzyme turning in 45° proton steps, with drawn protons, ADP/Pi/ATP and a grey membrane band. **shared**: the story page attaches it to its own box | own header |
