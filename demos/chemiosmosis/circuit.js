...
(function (global) {
  ...
  /* ---- Chemiosmosis, the name both components had ----
     Kept for the pages written against it: a create or mount dispatches on
     `context`, a mount rebuilds its box when the context changes, and
     Membrane's old circuit path gets a machine that dispatches the same
     way. Resolved at call time, so the components may load after this file. */
  const which = ctx => ctx === 'thylakoid' ? global.LightReactions : global.ElectronTransport;
  Object.assign(CHEM, {
    create: (THREE, root, camera, opts = {}) => which(opts.context).create(THREE, root, camera, opts),
    machine: eng => which(eng.P.context).machine(eng),
    get MACHINE_DEFAULTS() { return Object.assign({}, DEFAULTS, global.ElectronTransport ? global.ElectronTransport.DEFAULTS : {}, global.LightReactions ? global.LightReactions.DEFAULTS : {}); },
    SIGNALS, SCALE: SCALE(),
    mount(el, params = {}) {
      const P0 = Object.assign({}, params), subs = [];
      let ctx = P0.context === 'thylakoid' ? 'thylakoid' : 'mitochondrion', cur = null;
      const build = () => { cur = which(ctx).mount(el, P0); for (const s of subs) s.off = cur.on(s.ev, s.fn); };
      build();
      const handle = {
        get sim() { return cur.sim; }, get box() { return cur.box; }, get cell() { return cur.cell; },
        note: (n, o) => cur.note(n, o), notes: n => cur.notes(n), clearNotes: () => cur.clearNotes(),
        anchors: () => cur.anchors(), at: n => cur.at(n), layers: () => cur.layers(), palette: () => cur.palette(),
        show: (n, on) => { cur.show(n, on); return handle; },
        set(next) {
          const want = next.context === 'thylakoid' ? 'thylakoid' : next.context ? 'mitochondrion' : ctx;
          Object.assign(P0, next);
          if (want !== ctx) {
            ctx = want;
            const running = cur.box.running;
        ...
        state: () => cur.state(), signals: () => cur.signals(),
        on(ev, fn) {
          const s = { ev, fn, off: cur.on(ev, fn) };
          subs.push(s);
          return () => { if (s.off) s.off(); const i = subs.indexOf(s); if (i >= 0) subs.splice(i, 1); };
        },
        feed: (f, o) => cur.feed(f, o), spend: () => cur.spend(),
        add: (k, o) => cur.add(k, o), scatter: (k, n, s, o) => cur.scatter(k, n, s, o), clear: () => cur.clear(), reset: () => cur.reset(),
        start: () => cur.start(), stop: () => cur.stop(), pump: dt => cur.pump(dt),
        destroy: () => cur.destroy(),
      };
